...
varying float selfShadowVal;
      ...
      out float alpha)
  {
    ...
    #ifdef DIFFUSEMAP
      vec4 diffuseMapColor = texture2D(m_DiffuseMap, v_TexCoord);
      if(diffuseMapColor.a < m_AlphaDiscardThreshold){
        discard;
      }
    ...
    #ifdef SPECULARMAP
      specularColor = spec*texture2D(m_SpecularMap, v_TexCoord).rgb;
    #endif
    ...
    inout vec3 diffuseLightSum, inout vec3 specularLightSum)
  {
    ...
    diffuseLightSum += lightColor * max(0.0,NdotL) * attenuation;
    vec3 R = reflect(-L, N);
  ...
  void doPerFragmentLighting()
  {
    ...
    initializeMaterialColors(ambientColor, diffuseColor, specularColor, alpha);
...
void main (void)
{
    float noise = texture2D(m_AlphaNoiseMap, fadeVec.xy).r;
    float fadeVal = clamp((m_FadeEnd - fadeVec.z)/m_FadeRange,0.0,1.0);
