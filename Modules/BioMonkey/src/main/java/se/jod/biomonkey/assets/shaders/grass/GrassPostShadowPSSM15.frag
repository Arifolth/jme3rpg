...
vec4 main_multiSample(in int numSample){
    float depth = fetchTextureSample(m_DepthTexture,texCoord.st,numSample).r;//getDepth(m_DepthTexture,texCoord).r;
    vec4 color = fetchTextureSample(m_Texture,texCoord.st,numSample);
    ...
    vec4 worldPos = vec4(getPosition(depth,texCoord.st),1.0);
...
void main(){
    ...
    if(outFragColor.a <= m_AlphaDiscardThreshold){
        discard;
    }
}