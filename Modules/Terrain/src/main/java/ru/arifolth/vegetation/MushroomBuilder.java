...
public class MushroomBuilder implements BuilderInterface {
    ...
    public void accept(Spatial mushroomNode) {
        ...
        if (hit != null) {
            if (hit.getContactPoint().y > Constants.WATER_LEVEL_HEIGHT) {
                ...
                mushroomNode.setLocalTranslation(plantLocation.x, plantLocation.y, plantLocation.z);
                mushroomNode.setLocalRotation(new Quaternion().fromAngleAxis(Utils.getRandomNumberInRange(-6.5f, 6.5f) * FastMath.DEG_TO_RAD, new Vector3f(1, 0, 1)));
