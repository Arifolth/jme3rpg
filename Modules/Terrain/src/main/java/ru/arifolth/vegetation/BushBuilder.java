...
public class BushBuilder implements BuilderInterface {
    ...
    private List<Spatial> setupBushes() {
        ...
        for (int i = 0; i < bushTreesAmount; i++) {
            Node bushInstance = BushTypeEnum.getRandomBush();
            bushInstance.setLocalScale(1 + Utils.getRandomNumberInRange(1, 3), 1 + Utils.getRandomNumberInRange(1, 3), 1 + Utils.getRandomNumberInRange(1, 3));
    ...
    public void accept(Spatial bushNode) {
        ...
        if (hit != null) {
            if ((hit.getContactPoint().y > Constants.WATER_LEVEL_HEIGHT)) {
                ...
                bushNode.setLocalTranslation(plantLocation.x, plantLocation.y, plantLocation.z);
